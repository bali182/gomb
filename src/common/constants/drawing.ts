export const STROKE_COLOR = '#000000'
export const LIGHT_STROKE_COLOR = '#FFFFFF'
export const STITCH_LINE_STORKE_COLOR = '#000000'
export const STITCH_HOLE_COLOR = '#000000'
export const CARD_COLOR = '#1cabff'

export const SELECTED_STROKE_COLOR = '#0f6cbd'
export const SELECTED_STITCH_LINE_STROKE_COLOR = '#0f6cbdbf'
export const SELECTED_STITCH_LINE_HOLE_COLOR = '#0f6cbd'
export const SELECTED_HOLE_FILL_COLOR = '#0f6cbd7b'
export const SELECTED_HOLE_STROKE_COLOR = '#0f6cbd'

export const STROKE_THICKNESS = 0.3
export const VIEWBOX_PADDING = 10

export const COMPONENT_NAME_COLOR = '#000000'
export const COMPONENT_DIMENSIONS_COLOR = '#666666'

export const STITCH_LINE_LABEL_COLOR = '#000000'
export const STITCH_LINE_LABEL_BACKGROUND_COLOR = '#ffffff88'
export const STITCH_ROUTE_LABEL_HORIZONTAL_PADDING = 1.5
export const STITCH_ROUTE_LABEL_VERTICAL_PADDING = 0.3
export const STITCH_ROUTE_LABEL_ICON_GAP = 0.3
